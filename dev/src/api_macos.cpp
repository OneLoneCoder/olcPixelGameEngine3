...
//! START DECLARATION
// TODO: Correct the below header locations
...
// Default values and configuration settings with constexpr
...
// Objective-C method type encoding constants with constexpr
...
namespace ObjectiveCSEL {
    ...
    // Claude Sonnet 4 worked out this neat macro system for selectors!
    // Macro to declare a selector variable
    #define DECLARE_SEL(varName) static SEL varName = nullptr
    // --> static SEL allocSel = nullptr; <--

    // Macro to register a selector during initialization
    #define REGISTER_SEL(varName, constName) varName = sel_registerName(constName)
    // --> allocSel = sel_registerName(kAllocSel); <--
    
    // Macro to generate both declaration and registration for a selector
    #define DEFINE_SELECTOR(varName, constName) \
        DECLARE_SEL(varName); \
        static const auto register_##varName = [](){ REGISTER_SEL(varName, constName); return true; }
    // --> static SEL allocSel = nullptr; static const auto register_allocSel = [](){ allocSel = sel_registerName(kAllocSel); return true; }(); <--
    
    // Core selector registry - automatically declares and manages all selectors
    #define SELECTOR_REGISTRY \
        X(allocSel, kAllocSel) \
        X(initSel, kInitSel) \
        X(setDelegateSel, kSetDelegateSel) \
        X(releaseSel, kReleaseSel) \
        X(sharedApplicationSel, kSharedApplicationSel) \
        X(activateIgnoringOtherAppsSel, kActivateIgnoringOtherAppsSel) \
        X(setActivationPolicySel, kSetActivationPolicySel) \
        X(runSel, kRunSel) \
        X(applicationWillFinishLaunchingSel, kApplicationWillFinishLaunchingSel) \
        X(applicationDidFinishLaunchingSel, kApplicationDidFinishLaunchingSel) \
        X(applicationWillTerminateSel, kApplicationWillTerminateSel) \
        X(applicationDidBecomeActiveSel, kApplicationDidBecomeActiveSel) \
        X(applicationWillResignActiveSel, kApplicationWillResignActiveSel) \
        X(initWithContentRectSel, kInitWithContentRectSel) \
        X(stringWithUTF8StringSel, kStringWithUTF8StringSel) \
        X(setTitleSel, kSetTitleSel) \
        X(orderFrontRegardlessSel, kOrderFrontRegardlessSel) \
        X(setAcceptsMouseMovedEventsSel, kSetAcceptsMouseMovedEventsSel) \
        X(makeKeyAndOrderFrontSel, kMakeKeyAndOrderFrontSel) \
        X(makeKeyWindowSel, kMakeKeyWindowSel) \
        X(frameSel, kFrameSel) \
        X(setFrameSel, kSetFrameSel) \
        X(windowDidResizeSel, kWindowDidResizeSel) \
        X(windowWillCloseSel, kWindowWillCloseSel) \
        X(windowDidBecomeKeySel, kWindowDidBecomeKeySel) \
        X(windowDidResignKeySel, kWindowDidResignKeySel) \
        X(windowDidMiniaturizeSel, kWindowDidMiniaturizeSel) \
        X(windowDidDeminiaturizeSel, kWindowDidDeminiaturizeSel) \
        X(keyCodeSel, kKeyCodeSel) \
        X(charactersSel, kCharactersSel) \
        X(utf8StringSel, kUTF8StringSel) \
        X(locationInWindowSel, kLocationInWindowSel) \
        X(clickCountSel, kClickCountSel) \
        X(modifierFlagsSel, kModifierFlagsSel) \
        X(buttonNumberSel, kButtonNumberSel) \
        X(deltaXSel, kDeltaXSel) \
        X(deltaYSel, kDeltaYSel) \
        X(keyDownSel, kKeyDownSel) \
        X(keyUpSel, kKeyUpSel) \
        X(mouseDownSel, kMouseDownSel) \
        X(mouseUpSel, kMouseUpSel) \
        X(mouseDraggedSel, kMouseDraggedSel) \
        X(mouseMovedSel, kMouseMovedSel) \
        X(rightMouseDownSel, kRightMouseDownSel) \
        X(rightMouseUpSel, kRightMouseUpSel) \
        X(rightMouseDraggedSel, kRightMouseDraggedSel) \
        X(otherMouseDownSel, kOtherMouseDownSel) \
        X(otherMouseUpSel, kOtherMouseUpSel) \
        X(scrollWheelSel, kScrollWheelSel) \
        X(acceptsFirstResponderSel, kAcceptsFirstResponderSel) \
        X(becomeFirstResponderSel, kBecomeFirstResponderSel) \
        X(canBecomeKeyViewSel, kCanBecomeKeyViewSel) \
        X(needsPanelToBecomeKeySel, kNeedsPanelToBecomeKeySel) \
        X(drawRectSel, kDrawRectSel) \
        X(makeFirstResponderSel, kMakeFirstResponderSel) \
        X(initWithAttributesSel, kInitWithAttributesSel) \
        X(initWithFramePixelFormatSel, kInitWithFramePixelFormatSel) \
        X(setContentViewSel, kSetContentViewSel) \
        X(openGLContextSel, kOpenGLContextSel) \
        X(makeCurrentContextSel, kMakeCurrentContextSel) \
        X(setAutoresizingMaskSel, kSetAutoresizingMaskSel) \
        X(flushBufferSel, kFlushBufferSel) \
        X(displaySel, kDisplaySel) \
        X(CGLContextObjSel, kCGLContextObjSel) \
        X(setValuesSel, kSetValuesSel)
    
    // Generate selector declarations using X-macro pattern
    #define X(varName, constName) DECLARE_SEL(varName);
    SELECTOR_REGISTRY
    // --> static SEL allocSel = nullptr; static SEL initSel = nullptr; ... <--
    #undef X

    // Initialize all selectors using macro-based registry - called once at startup
    void initializeSelectors() {
        if (allocSel) return; // Already initialized
        
        // Initialize all selectors using the X-macro pattern
        #define X(varName, constName) REGISTER_SEL(varName, constName);
        SELECTOR_REGISTRY
        // --> allocSel = sel_registerName(kAllocSel); ... <--
        #undef X
    }

     // Initialization function (Thread-safe)
...
extern "C" {
    ...
    BOOL imageloader_loadFromFile(struct ImageLoader* self, const char* filePath) {
        ...
        self->hasAlpha    = ((BOOL(*)(id, SEL))objc_msgSend)(bitmapRep, hasAlphaSel);
